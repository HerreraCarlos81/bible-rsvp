...
|------|------------|--------|-------|---------|
| 2026-06-07 | 15:15:00 | Agent (Grok) | `css/styles.css`, `index.html`, `js/script.js`, `js/i18n.js`, `agent.md` | Replaced ugly native `confirm` alert after chapter finish with custom bottom-right toast + 5s animated timer bar + Yes/No buttons (auto-continues to next chapter on Yes via goToNextChapter + auto startRSVP). Changed logo from ✝ to 📖⚡ (open book + lightning symbol for speed reading). Fixed language switcher (🇧🇷/BR button) text color: unselected now white (#fff) for contrast on dark card-bg; active uses dark text on gold bg. Updated control docs + change log. Deployed to AWS (no commit yet per user). | 
